\documentclass[]{beamer}  % option [handout] for printing
%\includeonlyframes{c}
...
%\usepackage{natbib}
%\usepackage[citestyle=authoryear-comp,bibstyle=authoryear,sorting=ynt]{biblatex}
%\addbibresource{soc-stat-meet.bib}
...
library(knitr)
opts_chunk$set(echo = TRUE, fig.align = "center", size = "small")
@
...
%	I-priors perform equally well, and in some cases, better than various model selection techniques.
	...
	\begin{itemize}\setlength\itemsep{0.8em}
		\item[1] \textbf{Modelling aerobic fitness through some exercise data} ($n = 30$, $p = 6$) \cite{Kuo1998}
			\begin{itemize}
			...
			\end{itemize}
		\item[2] \textbf{Effects of air pollution on mortality rate} ($n = 60$, $p = 15$) \cite{McDonald1973}
			\begin{itemize}
			...
			\end{itemize}
		\item[3] \textbf{Factors affecting ozone depletion} ($n = 178$, $p = 12,90$) \cite{Casella2006}
			\begin{itemize}
...
\begin{frame}[label=c]{The $w$ I-prior model}
	...
	\begin{overlayarea}{\textwidth}{8.2cm}
		...
		\only<2|handout:0>{
			\item Putting this back into our regression model \eqref{eq:linmod}, we obtain the \textit{w I-prior} model
			\begin{align*}
				\begin{gathered}
...
<<iprior.demo1>>=
str(hsbsmall, width = 58, strict.width = "cut")
@
\end{lrbox}

\newsavebox{\ipriordemoaa}
\begin{lrbox}{\ipriordemoaa}
<<iprior.demo1a, eval = FALSE>>=
str(hsbsmall)
@
\end{lrbox}
...
<<iprior.demo5a, echo = FALSE, results = "hide", cache = TRUE>>=
mod2 <- ipriorOptim(kernL(mathach ~ ses, hsbsmall, model = list(kernel = "FBM")))
@

\newsavebox{\ipriordemof}
\begin{lrbox}{\ipriordemof}
<<iprior.demo6, eval = FALSE>>=
plot(
...
<<iprior.demo6a, echo = FALSE, results = "hide", cache = TRUE>>=
mod3 <- ipriorOptim(kernL(mathach ~ .^2, hsbsmall))
@

\newsavebox{\ipriordemog}
\begin{lrbox}{\ipriordemog}
<<iprior.demo7, eval = FALSE>>=
plot(
...
<<iprior.demo7a, echo = FALSE, results = "hide", cache = TRUE>>=
mod4 <- ipriorOptim(kernL(mathach ~ .^2, hsbsmall, model = list(kernel = "FBM")))
@

\newsavebox{\ipriordemoh}
\begin{lrbox}{\ipriordemoh}
<<iprior.demo8, echo = FALSE>>=
mod1.mse <- sum((fitted(mod) - mod$ipriorKernel$Y)^2) / 661
mod2.mse <- sum((fitted(mod2) - mod2$ipriorKernel$Y)^2) / 661
mod3.mse <- sum((fitted(mod3) - mod3$ipriorKernel$Y)^2) / 661
mod4.mse <- sum((fitted(mod4) - mod4$ipriorKernel$Y)^2) / 661
tab1 <- round(c(mod1.mse, mod2.mse, mod3.mse, mod4.mse), 3)
tab2 <- round(c(logLik(mod), logLik(mod2), logLik(mod3), logLik(mod4)), 2)
tab <- as.data.frame(rbind(tab1, tab2))
colnames(tab) <- c("Canonical", "FBM", "Can. w/ intr", "FBM w/ intr")
rownames(tab) <- c("MSE", "logLik")
tab
@
\end{lrbox}
...
\begin{frame}[fragile]{The R/iprior package}
	...
	\vspace{2em}
	\vspace{-2mm}
	<<iprior.demo4a, echo = FALSE, fig.height = 5, fig.width = 8.5, out.width = "98%">>=
	plot(mod, plots = "fitted")
	@
	}
	...
	\usebox{\ipriordemoe} \\
	\vspace{-2mm}
	<<iprior.demo5b, echo = FALSE, fig.height = 5, fig.width = 8.5, out.width = "98%">>=
	plot(mod2, plots = "fitted")
	@
	}	
	...
	\usebox{\ipriordemof} \\
	\vspace{-2mm}
	<<iprior.demo6b, echo = FALSE, fig.height = 5, fig.width = 8.5, out.width = "98%">>=
	plot(mod3, plots = "fitted")
	@
	}	
	...
	\usebox{\ipriordemog} \\
	\vspace{-2mm}
	<<iprior.demo7b, echo = FALSE, fig.height = 5, fig.width = 8.5, out.width = "98%">>=
	plot(mod4, plots = "fitted")
	@
	}	
...
\begin{frame}{Shrinkage properties of I-priors}
	...
	\vspace{-2mm}
	\begin{itemize}
		\onslide<1-|handout:1>{\item Comparison to ridge regression and Lasso}
		\only<1|handout:1>{
		\begin{align*}
			\begin{gathered}
		...
		\uncover<5-|handout:2>{
			\vspace{-6mm}
			\item The objective $g$-prior is also popular
			\[
	...
	\begin{itemize}
		...
		\begin{align}\label{eq:betaiprior2}
			\begin{gathered}
				...
				{\color{fu-red}
				\epsilon_i \sim \text{N}(0, \psi^{-1})}  \\
				{\color{fu-red} i=1,\dots,n} \\ 
				\\
				\text{\underline{Priors}} \\
				{\color[HTML]{377EB8} \alpha \sim \N(0, a^2)} \\
				{\color[HTML]{4DAF4A} \boldsymbol\beta \sim \N(\mathbf 0, \psi\boldsymbol\Lambda \mathbf X ^\top \mathbf X \boldsymbol\Lambda), \text{ where } \boldsymbol\Lambda = \text{diag}[\lambda_1, \dots, \lambda_p] } \\
				{\color[HTML]{984EA3} \psi, \lambda_1^{-2}, \dots,  \lambda_p^{-2} \sim \Gamma(c,d) } \\
			\end{gathered}
		\end{align}	
		...
		\begin{align*}
			...
			&\propto {\color{fu-red} f(\mathbf y | \alpha, \boldsymbol{\beta}, \psi)} {\color[HTML]{377EB8} f(\alpha)} {\color[HTML]{4DAF4A} f(\boldsymbol{\beta} | \psi, \boldsymbol{\lambda}) } {\color[HTML]{984EA3} f(\psi) f(\lambda_1)\cdots f(\lambda_p) }
		\end{align*}
		}
...
<<iprior.jags1, eval = TRUE, echo = FALSE, results = "hide", cache = TRUE>>=
### Key parameters
beta.true <- matrix(c(10, 0), nc=1)
sigma.true <- 2
corx <- 0.5
n <- 100
x.mean <- c(0, 0)
x.var <- matrix(c(1, corx, corx, 1), nrow = 2)

### Generate data
p <- length(beta.true)
set.seed(81016)
X <- rmvnorm(n, mean = x.mean, sigma = x.var)
colnames(X) <- paste0("X", 1:p)
Y <- X %*% beta.true + rnorm(n, mean = 0, sd = sigma.true)
Y <- as.vector(Y)
Xs <- scale(X, scale = FALSE)  # only center the data
XTXs <- t(Xs) %*% Xs
XTX.inv <- solve(XTXs)
...
<<iprior.jags2, eval = TRUE, size = "scriptsize">>=
mod <- "
  model {
    for (i in 1:n) {  
      Y[i] ~ dnorm(mu[i], psi)  
...
<<iprior.jags3, eval = TRUE, message = FALSE, warning = FALSE, cache = TRUE, size = "scriptsize">>=
(mod.fit <- run.jags(mod, n.chains = 4, sample = 2500, thin = 10, 
                     method = "parallel", n.sims = 4))
@
\end{lrbox}
...
\begin{frame}[fragile]{Estimation using JAGS}
	...
	\only<2|handout:2>{\vspace{-1mm} \usebox{\ipriorjagsb}}
	
	\only<3|handout:3>{\vspace{-2mm} \usebox{\ipriorjagsc}}
	
	\only<4|handout:4>{
		<<iprior.jags4, eval = TRUE, echo = FALSE, fig.height = 4.5>>=
		tmp <- as.mcmc.list(mod.fit)
		for(i in 1:4) tmp[[i]] <- mcmc(tmp[[i]], start = 1)
		mod.fit.ggs <- ggs(tmp)
		ggs_autocorrelation(mod.fit.ggs, family = "lambda")
		@
	}
	
	\only<5|handout:5>{
		\vspace{-1mm}
		<<iprior.jags5, eval = TRUE, echo = FALSE, fig.width = 9, fig.height = 5.5, out.width = "105%">>=
		ggs_traceplot(mod.fit.ggs, family = "lambda")
		@
	}
\end{frame}
...
\begin{frame}{Introduction}
	\begin{itemize}
		\item Introduced as Hybrid Monte Carlo \citeHMC{duane1987hybrid} for use in lattice models of quantum theory. Statistical applications started appearing sparsely in the 1990s.
		\item Development of HMC software (Stan) began in 2011, motivated by the difficulties faced when doing full Bayesian inference on multilevel generalised linear models.
...
\begin{frame}{Hamiltonian dynamics cont.}
	...
	\begin{itemize}
		...
		\end{align*}
		\onslide<5->{Steps 1-3 are repeated $L$ times.}
...
\begin{frame}{Hamiltonian Monte Carlo}
	...
	\only<4|handout:0>{
	<<phaseportrait1, echo = FALSE, fig.width = 9.5, fig.height = 4>>=
	...
	tmp1 <- phaseR::flowField(sho, mgp = c(1.8, 0.7, 0),
	                  x.lim = c(-3, 3), xlab = "Position (x)",
	                  y.lim = c(-3, 3), ylab = "Momentum (p)",
	...
	\only<5|handout:0>{
	<<phaseportrait2, echo = FALSE, fig.width = 9.5, fig.height = 4>>=
	sho <- function(t, y, parameters) {
	  x <- y[1]
	  p <- y[2]
	  m <- parameters
	...
	tmp2 <- phaseR::trajectory(sho,
	                   ...
	                   t.end = 1,
	                   parameters = 1,
	                   colour = iprior::ipriorColPal(1), lwd = 2)
	points(2, 0, lwd = 2)
	x <- tmp2$x[length(tmp2$x)]
	y <- tmp2$y[length(tmp2$x)]
	points(x, y, lwd = 2, col = iprior::ipriorColPal(1))
	...
	\only<6|handout:0>{
	<<phaseportrait3, echo = FALSE, fig.width = 9.5, fig.height = 4>>=	
	# Phase portrait
	...
	lines(c(x, x), c(y, 0), lwd = 2, lty = 3)
	points(x, 0, lwd = 2)
	lines(c(x, x), c(0, 1), lwd = 2, col= iprior::ipriorColPal()[2])	
	points(x, 1, lwd = 2, col = iprior::ipriorColPal(1))
	@
	}

	\only<7|handout:0>{
	<<phaseportrait4, echo = FALSE, fig.width = 9.5, fig.height = 4>>=	
	# Phase portrait
	...
	\only<8|handout:0>{
	<<phaseportrait5, echo = FALSE, fig.width = 9.5, fig.height = 4>>=	
	# Phase portrait
	...
	lines(c(x, x), c(0, 2.8), lwd = 2, col= iprior::ipriorColPal()[2])
	tmp2 <- phaseR::trajectory(sho,
	                           y0 = c(x, 2.8),
	                           t.end = 0.5,
	                           parameters = 1,
	...
	\uncover<9>{
	<<phaseportrait6, echo = FALSE, fig.width = 9.5, fig.height = 4>>=
	# Phase portrait
	...
	lines(c(x, x), c(0, -2.5), lwd = 2, col= iprior::ipriorColPal()[2])
	tmp2 <- phaseR::trajectory(sho,
	                           y0 = c(x, -2.5),
	                           t.end = 1.5,
	                           parameters = 1,
...
\newsavebox{\stana}
\begin{lrbox}{\stana}
<<stan.langa, eval = FALSE, size = "footnotesize">>=
stan.iprior.mod <- "
  function {
    ...
  }
...
\newsavebox{\stanb}
\begin{lrbox}{\stanb}
<<stan.langb, eval = FALSE, size = "footnotesize">>=
...
<<stan.lang, echo = FALSE, size = "scriptsize">>=
stan.mod <- "
  data {
    int n; // number of data
    int p; // number of parameters
...
<<stan.comp, cache = TRUE, results = "hide", warning = FALSE, message = FALSE>>=
...
<<stan.data, cache = TRUE, results = "hide", warning = FALSE, message = FALSE>>=
...
<<stansamp, cache = TRUE, results = "hide", warning = FALSE, message = FALSE>>=
...
\begin{frame}[fragile]{Stan example}
	\only<1|handout:1>{\vspace{-2mm} \usebox{\stana}}
	\only<2|handout:2>{\vspace{-2mm} \usebox{\stanb}}
	...
	\only<6|handout:4>{\vspace{-4mm} \usebox{\stanres}}
	
	\only<7|handout:5>{
	\vspace{-4mm}
	<<stanplot, eval = TRUE, echo = FALSE, fig.width = 9, fig.height = 5.5, out.width = "105%">>=
	a1 <- stan2coda(fit.stan)
	fit.ggs <- ggs(a1[, c("beta[1]", "beta[2]", "lambda[1]", "lambda[2]", "sigma")])
	# ggs_running(fit.ggs)
	ggs_autocorrelation(fit.ggs, family = "lambda")	
	@
	}
	
	\only<8|handout:6>{
	\vspace{-4mm}
	<<lambdastan, eval = TRUE, echo = FALSE, fig.width = 9, fig.height = 5.5, out.width = "105%">>=
	ggs_running(fit.ggs, family = "lambda")
	@
	}
...
\begin{frame}{Approach 1: Marginalise}
	...
	\begin{itemize}
		...
		\item \textit{Problem: For Bayesian Variable Selection models, this is intractable because need to sum over all $2^p$ models.}
	\end{itemize}
\end{frame}
...
\begin{frame}{Approach 3: Use Gibbs sampler}
	\begin{itemize}
		...
		\item \textit{Problem: Have to write code for the HMC sampler, which won't include all the automatic tuning that Stan has.}
	\end{itemize}
\end{frame}
...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Does the off-diagonal elements in the I-prior covariance matrix affect variable selection results in multicollinearity situations?
		\end{itemize}
		\pause
...
\texttt{@}
	...
	\begin{columns}
		...
		\end{column}
		\begin{column}{0.5\textwidth}
		<<chunk.name2, out.width = "100%", fig.width = 5, fig.height = 2.5>>=
		hist(x)
		@
		\end{column}
	\end{columns}
...
{\framenonumber
\begin{frame}[t,noframenumbering]{HMC References}	
	\addtocounter{framenumber}{-1}
...
\frametitle[lme4 references]{\texttt{lme4} References}	
	\bibliographystylelme{apalike}
	\nocitelme{*}
	\bibliographylme{soc-stat-lme}
\end{frame}
}
